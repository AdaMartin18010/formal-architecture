...
```text
场景: 客户端C发送SYN，网络延迟导致SYN在网络中滞留。
       C超时重发SYN'，服务端S对SYN'回复SYN'-ACK并建立连接。
       之后，延迟的SYN到达S，S再次建立无效连接。

结果: S维护了两个对应同一客户端的连接状态，
       其中一个永远处于半开状态，浪费内存和端口资源。

形式化: 设连接标识为四元组 ⟨SrcIP, SrcPort, DstIP, DstPort⟩。
        两次握手无法区分 "新连接请求" 与 "延迟的旧连接请求"。
        ∴ 需要第三次ACK使S确认C已收到SYN-ACK。∎
```

### 6.2 边界案例：Nagle算法与延迟ACK的致命交互

```text
Nagle算法 (RFC 896):
  条件: 若已有未确认数据，则缓冲小数据包直到:
        (a) 已发送数据被ACK，或 (b) 缓冲数据达到MSS

延迟ACK (Delayed ACK, RFC 1122):
  机制: 接收方不立即回复ACK，等待最多500ms或下一个数据包

交互死锁:
  发送方: 发送1字节数据，等待ACK才能发送下一字节 (Nagle)
  接收方: 收到1字节，等待更多数据才回复ACK (Delayed ACK)

结果: 双方互相等待，通信延迟增加约200-500ms。
      实时交互应用（如SSH、游戏）体验极差。

解决方案:
  - 设置 TCP_NODELAY 禁用Nagle (发送方)
  - 或设置 TCP_QUICKACK 禁用延迟ACK (接收方，Linux)
```

### 6.3 边界案例：CUBIC在WiFi网络中的公平性失衡
...
问题: CUBIC的三次函数增长在丢包率高的WiFi环境中过于激进。
      笔记本的大文件下载迅速占据cwnd优势，
      手机的视频流因频繁缓冲而卡顿。

定量: WiFi丢包率约1-5%，远高于有线网络的0.001-0.1%。
      CUBIC的W_max在WiFi中频繁重置，增长曲线不断重启，
      导致长流与短流之间的带宽分配严重失衡。

缓解: BBR在WiFi中表现更好（基于带宽模型而非丢包），
      但BBR与CUBIC共存时又会挤压CUBIC流。
```

### 6.4 失效条件：SYN Flood攻击下的状态机崩溃

```text
攻击者发送大量伪造源IP的SYN包，但不回复SYN-ACK。

服务端状态:
  每收到一个SYN → 创建SYN-RCVD状态条目
  每个条目占用内存 ≈ 256 bytes (Linux默认)
   backlog队列满 → 后续合法SYN被丢弃

形式化: 设攻击速率为 λ_attack，连接超时为 T_timeout。
        服务端最大并发半开连接数 = backlog。
        若 λ_attack × T_timeout > backlog，合法连接建立成功率 → 0。

防御 (SYN Cookies):
  服务端不维护SYN-RCVD状态，
  将连接状态编码到SYN-ACK的序列号中 (MSS、时间戳、密码学哈希)。
  收到ACK后，验证Cookie并重建状态。
```

### 6.5 2025-2026动态：BBRv2与TCP Prague

```text
BBRv2 (Google, 2020-2025):
  - 改进BBR与丢包算法的公平性
  - 引入ECN（显式拥塞通知）作为辅助信号
  - 在共享瓶颈时更保守，减少对CUBIC流的挤压
  - Linux 6.x 内核逐步集成

TCP Prague (IETF L4S工作组):
  - 目标: 实现低延迟、低丢包、可扩展吞吐量 (L4S)
  - 机制: ECN精确标记 + 更激进的拥塞响应
  - 与CUBIC/BBR共存于双队列AQM（PI2）中
  - 2025年状态: 实验性部署，CDN边缘节点试点
```
