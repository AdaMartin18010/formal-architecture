...
| **TUM Petri Nets (IN2052)** | [TUM 课程页](https://www.cs.cit.tum.de/tcs/lehre/ss25/petri-nets/)（Javier Esparza） | place-transition Petri 网、可达/覆盖图、不变量、展开；制造、协议、硬件 |
| 2025 对齐资源库（含自学资源） | [2025-对齐资源库-最新版](../2025-对齐资源库-最新版.md) §2.12–2.13 | CS2023 知识域简表、TUM Petri 网、Coursera 时序逻辑与模型检查等 |
...
- **学完本节应能**：（与自检一一对应）
  - 应能 1：用 FSM 或 LTL/CTL 写出简单形式规约（如安全/活性），并说明模型检测的输入/输出。
  - 应能 2：区分 LTL 与 CTL、FSM 与 Petri 网与 CCS/CSP 的适用场景与表达力。
  - 应能 3：说明模型检测与定理证明的适用场景及与本体系其他理论（形式语言、软件架构）的关系。
...
| **FSM vs CCS/CSP vs Petri 网** | FSM：单进程、显式状态；CCS/CSP：进程代数、行动/通道、组合并发；Petri 网：基于网/托肯的并发、资源共享。三者表达力与适用场景不同；统一状态转换系统（USTS）提供抽象统一 | [2. 状态机理论](#2-状态机理论)、[6. 进程代数理论](#6-进程代数理论)、[3. Petri网理论](#3-petri网理论)、[11-理论统一与整合总论](../11-理论统一与整合/07-理论统一与整合/00-理论统一与整合总论.md) |

**权威百科参考**：参见 [Wikipedia: Formal methods](https://en.wikipedia.org/wiki/Formal_methods)、[Wikipedia: Model checking](https://en.wikipedia.org/wiki/Model_checking)、[Wikipedia: Temporal logic](https://en.wikipedia.org/wiki/Temporal_logic)、[Wikipedia: Petri net](https://en.wikipedia.org/wiki/Petri_net)、[nLab: Formal methods](https://ncatlab.org/nlab/show/formal+methods)。
...
- **对齐状态**：已完成（最后更新：2025-02-10）
- **标准条款映射**：与形式化/架构/V&V 相关的 ISO/IEC 42010、25010、15288、12207、SEBoK、IEEE 1012、DIS 42024/42042 与本仓库文档的映射见 [13-项目报告与总结/00-标准条款与文档映射表](../13-项目报告与总结/00-标准条款与文档映射表.md)。
- **课程单元↔文档映射**：按课程节奏学习时可参考 [2025-对齐资源库 §2.11](../2025-对齐资源库-最新版.md#211-课程单元文档映射试点)（Stanford CS 256、CMU 15-414、MIT 6.1040 等）。

**完成度**：

- 理论基础：✅ 已完成
- 核心理论：✅ 已完成
- 应用实践：✅ 已完成（应用领域见 §1.3，工具与标准见 2025 对齐）
- 2025对齐：✅ 已完成（见本节 2025 对齐；与可持续推进节奏季度复审同步）
