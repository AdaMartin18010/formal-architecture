...
- [事件驱动架构理论](#事件驱动架构理论)
  ...
  - [2. 事件驱动架构核心概念](#2-事件驱动架构核心概念)
    - [2.1 事件（Event）](#21-事件event)
    - [2.2 事件生产者（Event Producer）](#22-事件生产者event-producer)
    - [2.3 事件消费者（Event Consumer）](#23-事件消费者event-consumer)
    - [2.4 事件总线（Event Bus）](#24-事件总线event-bus)
  - [3. 事件流处理理论](#3-事件流处理理论)
    - [3.1 事件流定义](#31-事件流定义)
    - [3.2 流处理操作](#32-流处理操作)
    - [3.3 时间语义](#33-时间语义)
  - [4. 事件溯源（Event Sourcing）](#4-事件溯源event-sourcing)
    - [4.1 事件溯源原理](#41-事件溯源原理)
    - [4.2 聚合根（Aggregate Root）](#42-聚合根aggregate-root)
    - [4.3 快照（Snapshot）](#43-快照snapshot)
  - [5. CQRS（命令查询职责分离）](#5-cqrs命令查询职责分离)
    - [5.1 CQRS原理](#51-cqrs原理)
    - [5.2 读写模型同步](#52-读写模型同步)
  - [6. 消息队列与事件总线](#6-消息队列与事件总线)
    - [6.1 消息队列模式](#61-消息队列模式)
    - [6.2 事件路由](#62-事件路由)
  - [7. 流处理与实时分析](#7-流处理与实时分析)
    - [7.1 流处理引擎](#71-流处理引擎)
    - [7.2 复杂事件处理（CEP）](#72-复杂事件处理cep)
  - [8. 微服务与事件驱动架构](#8-微服务与事件驱动架构)
    - [8.1 事件驱动微服务](#81-事件驱动微服务)
    - [8.2 Saga模式](#82-saga模式)
  - [9. 工程实践与最佳实践](#9-工程实践与最佳实践)
    - [9.1 事件设计原则](#91-事件设计原则)
    - [9.2 性能优化](#92-性能优化)
    - [9.3 监控与可观测性](#93-监控与可观测性)
  - [10. 未来发展趋势](#10-未来发展趋势)
    - [10.1 事件流平台](#101-事件流平台)
    - [10.2 事件驱动AI/ML](#102-事件驱动aiml)
    - [10.3 边缘计算](#103-边缘计算)
  - [11. 参考文献](#11-参考文献)
  - [12. 工程应用案例与最佳实践](#12-工程应用案例与最佳实践)
    - [1. 金融行业：实时风控与交易](#1-金融行业实时风控与交易)
    - [2. 电商平台：订单与库存解耦](#2-电商平台订单与库存解耦)
    - [3. IoT与智能制造：设备事件采集与响应](#3-iot与智能制造设备事件采集与响应)
    - [工程建议](#工程建议)
  - [2025 对齐](#2025-对齐)
