...
| 类型 | 模式 | 状态 | 说明 |
|------|------|------|------|
| 创建型 | 工厂方法 | ✅ 已完整 | 形式化描述 + 最小示例 + 与 06-形式模型/06-软件架构 链接 |
| 创建型 | 抽象工厂 | 待补充 | 每季度可补 1–2 个模式 |
| 创建型 | 建造者 | ✅ 已完整 | 形式化描述 + 最小示例 + 与 06-形式模型/06-软件架构 链接 |
| 创建型 | 原型 | 待补充 | |
| 创建型 | 单例 | ✅ 已完整 | 形式化描述 + 最小示例 + 与 06-形式模型/06-软件架构 链接 |
| 结构型 | 适配器 | ✅ 已完整 | 形式化描述 + 最小示例 + 与 06-形式模型/06-软件架构 链接 |
| 结构型 | 桥接 | 待补充 | |
| 结构型 | 组合 | 待补充 | |
| 结构型 | 装饰器 | 待补充 | |
| 结构型 | 外观 | 待补充 | |
...
| 行为型 | 迭代器 | 待补充 | |
| 行为型 | 中介者 | 待补充 | |
| 行为型 | 观察者 | ✅ 已完整 | 形式化描述 + 最小示例 + 与 06-形式模型/06-软件架构 链接 |
| 行为型 | 状态 | 待补充 | |
| 行为型 | 策略 | ✅ 已完整 | 形式化描述 + 最小示例 + 与 06-形式模型/06-软件架构 链接 |
| 行为型 | 解释器 | 待补充 | |

**维护**：建议每季度补 1–2 个模式（形式化描述 + 最小示例 + 与 [06-形式模型理论体系](../../06-形式模型理论体系/00-形式模型理论体系总论.md)、[06-软件架构理论体系](../../06-软件架构理论体系/00-软件架构理论体系总论-整合版.md) 的链接）。与 [00-可持续推进节奏](../../00-总览与导航/00-可持续推进节奏.md) 一致。

## 目录

- [设计模式详解-主题树形目录](#设计模式详解-主题树形目录)
  - [设计模式详解](#设计模式详解)
    - [模式完整度清单（已完整 / 待补充）](#模式完整度清单已完整--待补充)
  - [目录](#目录)
  - [1. 设计模式原则](#1-设计模式原则)
...
**形式化描述**：设产品类型为 \(P\)，构建步骤序列为 \(\sigma = (s_1, s_2, \ldots, s_n)\)。建造者定义接口 \(B\) 满足：\(\forall s_i \in \sigma,\, B\) 提供对应构建方法；Director 依序调用 \(B\) 的方法，最终由 \(B\) 返回 \(P\)。等价于 [06-形式模型理论体系](../../06-形式模型理论体系/00-形式模型理论体系总论.md) 中的**多步状态转换**：每步构建对应一次状态迁移，最终状态即完整产品。与 [06-软件架构理论体系/01-架构模式理论](../../06-软件架构理论体系/01-架构模式理论.md) 中的管道/流水线式组件一致。
...
class ConcreteBuilder implements Builder {
  ...
  void buildPartA() { p.setA(...); }
  void buildPartB() { p.setB(...); }
  Product getResult() { return p; }
}
// 使用: Director d = new Director(); Builder b = new ConcreteBuilder(); d.construct(b); Product p = b.getResult();
```

**相关**：[形式模型理论体系总论](../../06-形式模型理论体系/00-形式模型理论体系总论.md)（多步转换、状态累积）、[架构模式理论](../../06-软件架构理论体系/01-架构模式理论.md)（管道-过滤器风格）。
...
**形式化描述**：设类型 \(T\) 的实例集合在运行时至多为单元素：\(|\text{instances}(T)| \leq 1\)。单例等价于 [06-形式模型理论体系](../../06-形式模型理论体系/00-形式模型理论体系总论.md) 中的**全局唯一状态**：存在唯一状态 \(s \in S\) 对应该类型，所有获取操作返回同一 \(s\)。与 [06-软件架构理论体系/01-架构模式理论](../../06-软件架构理论体系/01-架构模式理论.md) 中的共享资源/注册表组件一致。
...
**相关**：[形式模型理论体系总论](../../06-形式模型理论体系/00-形式模型理论体系总论.md)（状态唯一性）、[架构模式理论](../../06-软件架构理论体系/01-架构模式理论.md)（共享组件）。
...
**形式化描述**：设算法族为 \(\mathcal{A}\)，上下文为 \(C\)。策略模式对应映射 \(\sigma : C \to \mathcal{A}\)，客户仅依赖「策略接口」\(f: X \to Y\)，具体算法可替换。与 [06-形式模型理论体系](../../06-形式模型理论体系/00-形式模型理论体系总论.md) 中的**行为多态/接口抽象**一致；在 [06-软件架构理论体系/01-架构模式理论](../../06-软件架构理论体系/01-架构模式理论.md) 中体现为可插拔策略组件。

**最小代码示例**（伪代码）：

```text
interface Strategy { int execute(int a, int b); }
class Add implements Strategy { int execute(int a, int b) { return a + b; } }
class Multiply implements Strategy { int execute(int a, int b) { return a * b; } }
class Context {
  private Strategy s;
  Context(Strategy s) { this.s = s; }
  int run(int a, int b) { return s.execute(a, b); }
}
// 客户: new Context(new Add()).run(2,3) 或 new Context(new Multiply()).run(2,3)
```

**相关**：[形式模型理论体系总论](../../06-形式模型理论体系/00-形式模型理论体系总论.md)（接口与行为等价）、[架构模式理论](../../06-软件架构理论体系/01-架构模式理论.md)（可替换组件）。
