...
- 关联：`00-编程语言理论统一总论.md` ｜ `04-编译统一理论.md` ｜ `05-运行时理论.md`
- 邻接主题：`03-类型统一理论.md`（效果系统、线性/仿射类型）

### 快速阅读路径

- 工程落地：2（基础/同步与通信）→ 3（实现）→ 4（算法）
- 理论导向：1（定义/模型）→ 2.4（内存模型与 hb）→ 6（趋势）
...
### 2.4 内存模型与 happens-before（示例）

- 程序次序规则（program order）：同一线程内的语句按程序顺序建立先发生关系
- 同步次序（synchronizes-with）：解锁→加锁、写入释放→读取获取、线程启动/加入 等建立跨线程的先发生关系
- 传递性：hb 关系可传递组合
...
let t = thread::spawn(move || {
    ...
    *ready = true;            // 写入（在释放锁前）
    cvar.notify_one();        // synchronizes-with 后续 wait 唤醒
});

let (lock, cvar) = &*data;
let mut ready = lock.lock().unwrap();
while !*ready {
    ready = cvar.wait(ready).unwrap(); // 获取唤醒，建立 hb
}
// 此处观测到 *ready == true 有 happens-before 保证
...
wg.Add(1)
go func(){
    atomic.StoreUint32(&ready, 1) // release
    wg.Done()
}()
wg.Wait() // join 建立 hb
...
- Mutex/Semaphore/CV：互斥锁/信号量/条件变量
- Happens-before：先发生关系；Race：数据竞争
- Deadlock/Livelock/Starvation：死锁/活锁/饥饿
- Channel/Actor：通道/参与者模型要素
- SLO：并发系统延迟/吞吐/错误率目标
